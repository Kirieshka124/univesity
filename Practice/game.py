from modules.manager import FileManager
...
while True:
    ...
    difficulty = input("Выберите режим (легкий, средний, сложный): ").lower()
    if difficulty == "легкий":
        ...
        print("Неверный режим. Игра будет запущена на легком уровне.")
        lives = 7
        ...
        try:
            ...
            if len(guess) > 1:
                if guess == game.get_word():
                    hidden_word = list(game.get_word())
                    print("Вы угадали слово!")
                    break
                else:
                    lives = 0
                    print("Неверно! Вы потеряли все жизни.")
                    break
